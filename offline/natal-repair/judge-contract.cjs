// Shared source embedded verbatim in both Code nodes and both model prompts.
...
const shape={schemaVersion,hardGates:Object.fromEntries(hardGates.map(k=>[k,true])),scores:Object.fromEntries(scores.map(k=>[k,4])),verdict:'pass|revise|reject',failedSections:[],issues:[{section:'exact allowed section ID',category:categories.join('|'),repairable:true,problem:'short exact problem',requiredFix:'specific correction',factIds:['exact supplied authoritative fact ID']}],flags:[],notes:'short reason'};
...
module.exports={schemaVersion,hardGates,scores,categories,shape,validateJudge};
