...
const d=$('Build Rejudge Prompt').first().json;
const raw=$input.first().json.text??$input.first().json.output??$input.first().json.content??'';
try{const result=validateJudge(raw,d);return [{json:{...d,rejudge:result.judge,rejudgeContractError:false,judgeNeedsRevision:false,attempt:3}}];}
catch(_){return [{json:{...d,rejudge:null,rejudgeContractError:true,judgeNeedsRevision:false,attempt:3}}];}