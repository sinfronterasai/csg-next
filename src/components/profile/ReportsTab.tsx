    ...
    return (
      <div className="glass-panel glow-border rounded-2xl p-12 text-center">
        ...
        <p className="text-cosmic-200">
          Your yearly transits, vocation, and relationship reports will appear here.
        </p>
      </div>
    );
  }
      ...
      {reports.map((report) => (
        <div key={report.id} className="glass-panel glow-border rounded-2xl overflow-hidden">
          ...
          >
            ...
            <i
              className={`fa-solid fa-chevron-${expanded === report.id ? 'up' : 'down'} text-gold transition-transform`}
            ></i>
          </button>
          {expanded === report.id && (
            <div className="px-6 pb-6 border-t border-gold/20 pt-4">
              {report.result.text && (
                <div className="prose prose-invert max-w-none text-cosmic-100 leading-relaxed whitespace-pre-line">
                  {report.result.text}
                </div>
              )}
              {report.pricePaid != null && (
                <p className="mt-4 text-sm text-cosmic-300">
                  <i className="fa-solid fa-tag mr-2"></i>
                  ${report.pricePaid.toFixed(2)}
                </p>
              )}
            </div>
          )}
