    ...
    return (
      <div className="glass-panel glow-border rounded-2xl p-12 text-center">
        ...
        <p className="text-cosmic-200 mb-6">
          Your tarot journal will grow with each reading. Draw your first card to begin.
        </p>
        <Link
          href="/tarot"
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <Link
          key={item.id}
          href={`/tarot/reading/${item.id}`}
          className="glass-panel glow-border rounded-2xl p-5 block hover:border-gold transition-all duration-300 group"
        >
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h4 className="font-serif text-lg font-semibold text-gold group-hover:text-cosmic-100 transition">
                {item.question}
              </h4>
