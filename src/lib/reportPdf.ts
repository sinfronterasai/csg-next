...
export async function downloadYearlyTransitIcs(readingId: number): Promise<boolean> {
  const response = await fetch(`/api/reports/${readingId}/ics`);
  if (!response.ok) return false;
  const blob = await response.blob();
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `cosmic-spirit-guide-yearly-transit-${readingId}.ics`;
  link.click();
  URL.revokeObjectURL(url);
  return true;
}

export async function downloadYearlyTransitPdf(readingId: number): Promise<boolean> {
  const response = await fetch(`/api/reports/${readingId}/pdf`);
  if (!response.ok) return false;
  const blob = await response.blob();
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `cosmic-spirit-guide-yearly-transit-${readingId}.pdf`;
  link.click();
  URL.revokeObjectURL(url);
  return true;
}
