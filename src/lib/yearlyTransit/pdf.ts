...
export async function buildYearlyTransitPdf(pack: YearlyTransitFactPack, title: string, name: string, sections: Array<{ heading: string; body: string }> = []): Promise<Uint8Array> {
  ...
  const draw = (text: string, size = 10, font = regular, color = rgb(0.15, 0.12, 0.2)) => { if (y < 52) { page = pdf.addPage([612, 792]); y = 742; } page.drawText(winAnsiText(text), { x: 42, y, size, font, color }); y -= size + 6; };
