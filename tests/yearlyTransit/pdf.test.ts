...
describe('yearly transit PDF artifact', () => {
  it('creates a readable non-empty PDF from the immutable pack', async () => {
    const pdf = await buildYearlyTransitPdf(buildWorstCaseYearlyTransitPack(), 'Yearly Transit Forecast', 'Seeker', [{ heading: 'Overall Theme', body: 'Evidence-backed interpretation.' }]);
